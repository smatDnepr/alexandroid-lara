<div>
	...
	<div class="card-body items-list sortable-list landing-list p-0 livevire-component-card-body" data-suffix="landing_promo_slides">
		@foreach ($slides as $index => $slide)
			<div class="item landing-item sortable-item" data-id="{{ $slide->id }}">
				<form wire:key="form_landing_slide_{{ $index }}" wire:submit.prevent="updateItem({{ $index }}, {{ $slide->id }})" class="form-landing-slide">
					<div class="row">
						<div class="col-sm-1">
							<div class="form-group">
								<label>{{ __('Картинка') }}</label>
								<input type="hidden" id="img_input_hidden__landing_promo_slide_{{ $index }}" wire:model="img.{{ $index }}" />
								<div class="img-wrap">
									<a class="img" href="{{ $img[$index] }}" target="_blank">
										<img class="thumb" src="{{ $thumb[$index] }}" />
									</a>
								</div>
								<a href="#" data-toggle="modal" data-target="#rfm_img_modal__landing_promo_slide_{{ $index }}">{{ __('Изменить') }}</a>
							</div>
						</div>
						<div class="col-sm-5">
							<div class="form-group">
    ...
    <div class="modals-wrap">
		@foreach ($slides as $index => $slide)
			<div class="modal fade" id="rfm_img_modal__landing_promo_slide_{{ $index }}" tabindex="-1" role="dialog" aria-labelledby="rfm_modal" aria-hidden="true">
				<div class="modal-dialog modal-dialog-centered modal-xl" role="document">
					<div class="modal-content">
						<iframe src="/vendor/filemanager/dialog.php?type=1&field_id=img_input_hidden__landing_promo_slide_{{ $index }}" frameborder="0" width="100%" height="630"></iframe>
					</div>
				</div>
			</div>
		@endforeach
